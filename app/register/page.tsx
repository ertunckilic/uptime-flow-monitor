      ...
      <div className="flex items-center justify-center min-h-[80vh] p-6">
        <div className="w-full max-w-sm">
          ...
          <form action={registerUser} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-white text-black font-medium py-3 rounded-lg hover:bg-neutral-200 transition-colors mt-2 active:scale-[0.98]">
              {t.auth.regBtn}
            </button>
          </form>

          <p className="text-center mt-8 text-sm text-neutral-500">
            {t.auth.hasAccount} <Link href="/login" className="text-white hover:underline font-medium">{t.auth.loginBtn}</Link>
          </p>
        </div>
      </div>
    </div>
